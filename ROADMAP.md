...
## Phase 4 (Risk scoring calibration)
- [ ] Popularity-weighted heuristic dampening — high-popularity packages (high dependent count, many published versions) should have heuristic signal scores scaled down rather than reported at face value. A `child_process` hit or install script in a package with tens of millions of weekly downloads is categorically different from the same signal in an obscure package. The current engine only adds a low-popularity *penalty*; it needs a high-popularity *damper* so mainstream developer tooling (webpack-dev-server, eslint plugins, test runners) does not generate routine false-positive risk alerts.
- [ ] Version age dampening — a package version that has been published for an extended period (e.g. > 6 months) without an OSV advisory has had significant community and automated scrutiny. The cooldown policy already tracks publication date; the risk engine should use it to progressively reduce heuristic scores for older versions, down to a configurable floor. Newly published versions of any package — including popular ones — should retain full signal weight.
