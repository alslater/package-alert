...
- **SSH credential isolation:** `~/.ssh` is hidden inside the sandbox home tmpfs by default. It is re-exposed read-only only when `--expose-ssh-keys` is explicitly passed. package-alert detects SSH VCS dependencies (both `git+ssh://` and scp-style `git@host:org/repo` URLs) and prompts for the flag rather than silently failing.
