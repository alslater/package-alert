...
def test_classify_uv_wheel_index_build_tag(lang: PythonLanguage, tmp_path: Path) -> None:
    """Regression: a wheel with a PEP 427 build tag (e.g. a timestamp-style
    tag some reproducible-build pipelines use) is indexed by uv as
    "<version>-<build>-<python>-<abi>-<platform>" — 4 hyphen-delimited
    fields after the version, not 3. _UV_WHEEL_INDEX_TAGS_RE previously
    only accepted the no-build-tag shape, so this leaf was rejected
    outright and produced no cache event at all — confirmed against uv's
    actual source (WheelFilename::cache_key(), whose Display impl for a
    wheel with a build tag writes the tag repr verbatim, build tag
    included). See _UV_WHEEL_INDEX_TAGS_RE's own comment for the full
    mechanism.
    """
    entry = tmp_path / "wheels-v6" / "pypi" / "somepkg" / "1.2.3-202206090410-py3-none-any"
    entry.parent.mkdir(parents=True)
    entry.touch()
    result = lang.classify_cache_file(entry)
    assert result is not None
...
def test_classify_uv_wheel_index_tags_rejects_non_pep425_characters(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """A malformed cache entry containing a non-tag character (e.g. "@") in
    the python, abi, or platform field must be rejected, not classified as
    a real install — those fields are restricted to real PEP 425 tag
    characters just like parsers/wheel.py's _WHEEL_RE is.
    """
    entry = tmp_path / "wheels-v6" / "pypi" / "pkg" / "1.0-py3@-none-any"
    entry.parent.mkdir(parents=True)
    entry.touch()
...
@pytest.mark.parametrize(
    ("dist_dir", "leaf", "expected_name", "expected_version"),
    [
        ("zope.interface", "zope_interface-6.0-py3-none-any", "zope-interface", "6.0"),
        (
            "backports.zoneinfo",
            "backports_zoneinfo-0.2.1-py3-none-any",
            "backports-zoneinfo",
            "0.2.1",
        ),
        ("ruamel.yaml", "ruamel_yaml-0.18.6-py3-none-any", "ruamel-yaml", "0.18.6"),
    ],
)
def test_classify_uv_wheel_index_legacy_dotted_name_prefix(
    lang: PythonLanguage,
    tmp_path: Path,
    dist_dir: str,
    leaf: str,
    ...
    """Regression: a legacy wheels-v4 leaf embeds the distribution name in
    its WHEEL FILENAME spelling, which PEP 503/427 escape by collapsing
    every run of [-_.] to "_" — so a DOTTED name appears in the leaf as
    "zope_interface" while its parent directory is "zope.interface".

    The strip loop previously tried only the raw directory name and a
    naive "-"->"_" swap, so neither candidate matched a dotted name. The
    UNSTRIPPED leaf was then parsed, and _UV_WHEEL_INDEX_TAGS_RE matched
    it with version="zope_interface" — reading the NAME as the version and
    absorbing the real version as a build tag. That is a bogus OSV query
    and an alert for a version that does not exist, for packages as common
    as zope.interface. Confirmed empirically.
    """
    entry = tmp_path / "wheels-v4" / "pypi" / dist_dir / leaf
    entry.parent.mkdir(parents=True)
    entry.touch()
    result = lang.classify_cache_file(entry)
    assert result is not None, f"expected {leaf!r} under {dist_dir!r} to classify"
    assert result.name == expected_name
    assert result.version == expected_version, (
        f"expected the real version, not the escaped name read as one; "
        f"got {result.version!r}"
    )


def test_classify_uv_wheel_index_name_equals_version_not_misread_as_legacy_prefix(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """Regression: the legacy-schema name-prefix strip above used to be
    unconditional — if the leaf started with "<name>-", that prefix was
    always stripped before parsing. For a package whose name and RESOLVED
    VERSION happen to be identical (e.g. name="1", version="1"), the
    current-schema leaf "1-py3-none-any" also happens to start with the
    name prefix "1-", even though this is NOT a legacy wheels-v4-style
    leaf at all — stripping it left "py3-none-any", which has too few
    hyphen-separated fields to match the tags regex, so classification
    returned None and the install was silently missed. The fix only
    adopts the stripped form if it actually matches one of the two
    supported leaf schemas; otherwise it falls back to parsing the
    original, unstripped leaf — which correctly parses this shape as
    version="1" via the same tags regex.
    """
    entry = tmp_path / "wheels-v6" / "pypi" / "1" / "1-py3-none-any"
    entry.parent.mkdir(parents=True)
    entry.touch()
    result = lang.classify_cache_file(entry)
    assert result is not None
...
@pytest.mark.parametrize("suffix", [".http", ".msgpack", ".rev", ".lock"])
def test_classify_uv_wheel_index_symlinked_leaf_with_companion_suffix_still_classifies(
    lang: PythonLanguage, tmp_path: Path, suffix: str
) -> None:
    """Regression: rejecting a leaf purely because its name ends in a
    companion suffix was a real cache-detection bypass. A wheel's platform
    field is a PEP 425 compressed tag set whose components are constrained
    only by character class, and uv preserves an UNRECOGNISED component
    verbatim (WheelFilename::parse() stores the original tag string as
    WheelTagLarge.repr; parse_large_tag_component() silently skips any
    component it can't parse rather than erroring). So
    "evil-1.0.0-py3-none-any.http.whl" parses fine, stays installable —
    its parsed platform set still contains the compatible "any" — and its
    cache_key() yields the index leaf "1.0.0-py3-none-any.http". A
    suffix-only check discarded that, so the install produced no cache
    event at all.

    The discriminator is structural: a real index entry is the archive-v0
    symlink uv creates via Cache::persist_with_id, while every companion
    is an ordinary regular file (verified against a real populated cache:
    0 of 6,974 companion-suffixed paths were symlinks, and all 1,442
    genuine index entries were). A symlinked leaf must therefore still
    classify, even when its platform tag happens to end in one of these
    words.
    """
    archive = tmp_path / "archive-v0" / "somehash"
    archive.mkdir(parents=True)
    parent = tmp_path / "wheels-v6" / "pypi" / "evilpkg"
    parent.mkdir(parents=True)
    entry = parent / f"1.0.0-py3-none-any{suffix}"
    entry.symlink_to(archive, target_is_directory=True)

    result = lang.classify_cache_file(entry)
    assert result is not None, (
        f"expected a symlinked index entry whose platform tag ends in "
        f"{suffix!r} to still classify — suppressing it is a cache-"
        f"detection bypass for an installable wheel"
    )
    assert result.name == "evilpkg"
...
def test_classify_uv_sdist_bundled_src_artifacts_are_not_installs(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """Regression: an artifact BUNDLED inside a source distribution is file
    content, not an installed package.

    uv unpacks each source build's own sources into a `src/` directory inside
    the revision shard, and cache_file_globs()' artifact patterns are recursive
    (**/*.whl, **/*.dist-info, **/*.tar.gz), so they also match whatever the
    sdist happens to ship — test fixtures, vendored wheels, nested sdists.
    Classifying one reported a package that was never installed: confirmed
    empirically, a bundled vendor-9.9-py3-none-any.whl was reported as
    "vendor 9.9".

    Verified against a real populated cache: uv writes the BUILT wheel directly
    into the revision shard, as a SIBLING of `src/` — never inside it — so
    excluding src/ descendants cannot hide a genuine build output, which this
    test also pins.
    """
    shard = (
        tmp_path / ".cache" / "uv" / "sdists-v9" / "pypi" / "realpkg" / "1.0"
        / ("a" * 16)
    )
    bundled_dir = shard / "src" / "fixtures"
    bundled_dir.mkdir(parents=True)

    for name in (
        "vendor-9.9-py3-none-any.whl",
        "vendored-2.0.tar.gz",
    ):
        artifact = bundled_dir / name
        artifact.write_bytes(b"")
        assert lang.classify_cache_file(artifact) is None, (
            f"{name} is bundled source content, not an installed package"
        )

    bundled_distinfo = bundled_dir / "vendor-9.9.dist-info"
    bundled_distinfo.mkdir()
    assert lang.classify_cache_file(bundled_distinfo) is None

    # The genuine build output sits beside src/, and must still classify.
    built = shard / "realpkg-1.0-py3-none-any.whl"
    built.write_bytes(b"")
    metadata = lang.classify_cache_file(built)
    assert metadata is not None, "the real built wheel must still be detected"
    assert (metadata.name, metadata.version) == ("realpkg", "1.0")


def test_src_exclusion_does_not_suppress_a_package_named_src(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """Regression: `src` is a real PyPI package (0.0.7), and its own cache
    entries put that component at the NAME position.

    A plain `"src" in parts` membership check therefore suppressed both its
    built wheel and its version directory —
    sdists-v*/pypi/src/<version>/<revision>/src-<version>-*.whl — so
    installing it produced no event at all. uv's unpacked source tree only
    ever appears as a child of the revision shard, which always follows
    <name>/<version>, so the check skips that many components. The name
    offset is bucket-dependent, so both pypi/ and index/ are covered here.
    """
    revision = "a" * 16
    cache = tmp_path / ".cache" / "uv" / "sdists-v9"

    for rel in (
        f"pypi/src/1.0/{revision}/src-1.0-py3-none-any.whl",
        f"index/{revision}/src/1.0/{revision}/src-1.0-py3-none-any.whl",
    ):
        built = cache / rel
        built.parent.mkdir(parents=True, exist_ok=True)
        built.write_bytes(b"")
        metadata = lang.classify_cache_file(built)
        assert metadata is not None, f"a package named 'src' must classify: {rel}"
        assert (metadata.name, metadata.version) == ("src", "1.0")

    version_dir = cache / "pypi" / "src" / "1.0"
    metadata = lang.classify_cache_file(version_dir)
    assert metadata is not None, "its version directory must classify too"
    assert (metadata.name, metadata.version) == ("src", "1.0")

    # It must still have its OWN bundled content suppressed, and an artifact
    # sitting directly in a src/ tree (not only nested deeper) is bundled too.
    for rel in (
        f"pypi/src/1.0/{revision}/src/fixtures/vendor-9.9-py3-none-any.whl",
        f"pypi/realpkg/1.0/{revision}/src/vendor-9.9-py3-none-any.whl",
        f"index/{revision}/realpkg/1.0/{revision}/src/vendor-9.9-py3-none-any.whl",
    ):
        bundled = cache / rel
        bundled.parent.mkdir(parents=True, exist_ok=True)
        bundled.write_bytes(b"")
        assert lang.classify_cache_file(bundled) is None, (
            f"bundled source content must stay suppressed: {rel}"
        )


def test_src_exclusion_is_scoped_to_uv_sdist_roots(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """Regression: the bundled-src guard must not match a user's own path.

    An unscoped `"src" in path.parts` check also matched ordinary project
    layouts — a venv under `/project/src/.venv/lib/.../site-packages/` stopped
    classifying entirely, so installs in ANY project whose path merely contains
    a `src` component produced no events at all. Confirmed empirically. That is
    a silent miss, far worse than the false event the guard exists to prevent,
    so it is scoped to a validated sdists-v<digits> root and only applies to a
    `src` component BELOW it.
    """
    for rel in (
        "proj/.venv/lib/python3.12/site-packages/requests-2.31.0.dist-info",
        "project/src/.venv/lib/python3.12/site-packages/requests-2.31.0.dist-info",
        "srv/src/app/.venv/lib/python3.12/site-packages/requests-2.31.0.dist-info",
    ):
        dist_info = tmp_path / rel
        dist_info.mkdir(parents=True)
        (dist_info / "METADATA").write_text("Name: requests\nVersion: 2.31.0\n")
        metadata = lang.classify_cache_file(dist_info)
        assert metadata is not None, (
            f"a real install must still classify when its own path contains a "
            f"'src' component: {rel}"
        )
        assert (metadata.name, metadata.version) == ("requests", "2.31.0")

    # A `src` ABOVE the sdists-v* root is likewise the user's own path, not
    # uv's unpacked source tree, so the build output below it still classifies.
    shard = (
        tmp_path / "src" / ".cache" / "uv" / "sdists-v9" / "pypi" / "realpkg"
        / "1.0" / ("a" * 16)
    )
    shard.mkdir(parents=True)
    built = shard / "realpkg-1.0-py3-none-any.whl"
    built.write_bytes(b"")
    assert lang.classify_cache_file(built) is not None, (
        "a 'src' above the sdists-v* root must not suppress a real build output"
    )
...
def test_classify_uv_sdist_index_hash_wrong_width_not_misread_as_digest(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """uv's cache_digest() is always exactly 16 hex chars — a shorter or
    longer hex string in the digest position is not a real uv cache
    entry and must not classify, even though the surrounding shape
    (index/<hash>/<name>/<version>) otherwise matches.
    """
    too_short = tmp_path / "sdists-v9" / "index" / "deadbeef" / "totallyfake" / "9.9.9"
    too_short.parent.mkdir(parents=True)
    too_short.mkdir()
    assert lang.classify_cache_file(too_short) is None

    too_long = tmp_path / "sdists-v9" / "index" / "b2a7eb67d4c26b82ab" / "totallyfake2" / "9.9.9"
    too_long.parent.mkdir(parents=True)
    too_long.mkdir()
    assert lang.classify_cache_file(too_long) is None


def test_classify_uv_wheel_index_hash_wrong_width_not_misread_as_digest(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    """Same as test_classify_uv_sdist_index_hash_wrong_width_not_misread_as_digest,
    for the wheels-v*/index/<hash>/<name>/<leaf> shape.
    """
    entry = tmp_path / "wheels-v6" / "index" / "deadbeef" / "totallyfake" / "totallyfake-9.9.9-py3-none-any"
    entry.parent.mkdir(parents=True)
    entry.mkdir()
    assert lang.classify_cache_file(entry) is None
...
@pytest.mark.parametrize(
    ...
    [
        ("_UV_CACHE_HASH_RE", "deadbeefdeadbeef\n"),
        ("_UV_CACHE_SCHEMA_DIR_RE", "wheels-v6\n"),
        ("_UV_WHEEL_INDEX_BUILD_HASH_RE", "1.0.0-deadbeefdeadbeef\n"),
        ("_UV_WHEEL_INDEX_TAGS_RE", "1.0.0-py3-none-any\n"),
        ("_DISTINFO_RE", "pkg-1.0.0.dist-info\n"),
    ],
)
def test_uv_shape_regexes_reject_a_trailing_newline(pattern_name: str, candidate: str) -> None:
    r"""Regression: these patterns are documented as EXACT shape validators,
    but `$` in Python also matches immediately before a final newline, so
    every one of them accepted a candidate with a trailing newline.

    Linux permits newlines in filenames (only `/` and NUL are banned), so
    this is reachable rather than theoretical — a directory literally named
    `deadbeefdeadbeef\n` passed `_UV_CACHE_HASH_RE` and the whole subtree
    was then read as a real uv index path, producing a false package
    classification. `_UV_CACHE_SCHEMA_DIR_RE` is the most consequential:
    it gates which roots get a RECURSIVE INOTIFY WATCH, and its own comment
    notes that admitting a lookalike "could reopen the inotify watch
    exhaustion this scoping was meant to prevent".

    Anchored with `\Z` (which has no such newline exemption) on the
    patterns themselves rather than by converting ~19 `.match()` call sites
    to `.fullmatch()`, so a future call site cannot silently reintroduce it.
    """
    import packagealert.languages.python as python_mod

    pattern = getattr(python_mod, pattern_name)
    assert pattern.match(candidate) is None, (
        f"{pattern_name} must reject {candidate!r}: `$` allows a trailing "
        f"newline, so this is not the exact shape check it claims to be"
    )
    # The same value without the newline is still accepted, so the anchor
    # tightened the pattern rather than breaking it.
    assert pattern.match(candidate.rstrip("\n")) is not None, (
        f"{pattern_name} must still accept the legitimate {candidate.rstrip()!r}"
    )


def test_classify_uv_index_entry_under_newline_digest_dir_is_rejected(
    lang: PythonLanguage, tmp_path: Path
) -> None:
    r"""End-to-end companion to the regex test above: a directory whose name
    is a valid digest plus a trailing newline must not be read as a real uv
    index path. Before the `\Z` anchor it classified identically to a
    genuine digest, inventing an install from an unrelated subtree.
    """
    entry = (
        tmp_path / "wheels-v6" / "index" / "deadbeefdeadbeef\n"
        / "evilpkg" / "1.0.0-py3-none-any"
    )
    entry.parent.mkdir(parents=True)
    entry.touch()
    assert lang.classify_cache_file(entry) is None, (
        "a newline-suffixed digest directory must not be treated as a real "
        "uv index path"
    )

    # Control: the same shape under a genuine digest still classifies.
    good = (
        tmp_path / "wheels-v6" / "index" / "deadbeefdeadbeef"
        / "goodpkg" / "1.0.0-py3-none-any"
    )
    good.parent.mkdir(parents=True)
    good.touch()
    result = lang.classify_cache_file(good)
    assert result is not None and result.name == "goodpkg"
